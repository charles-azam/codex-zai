...
//!
//! The history is stored at `~/.codex-zai/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.codex-zai`.
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.codex-zai/history.jsonl` and ensure the parent directory exists.
