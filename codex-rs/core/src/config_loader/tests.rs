...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".codex-zai")).await?;
    tokio::fs::create_dir_all(project_root.join(".codex-zai")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".codex-zai").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".codex-zai").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    assert_eq!(
        ...
        nested.join(".codex-zai").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".codex-zai").as_path()
    );
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".codex-zai").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".codex-zai").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".codex-zai").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn codex_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    ...
    let codex_home = home_dir.join(".codex-zai");
    tokio::fs::create_dir_all(&codex_home).await?;
...
async fn codex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".codex-zai");
    let nested_dot_codex = nested.join(".codex-zai");
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".codex-zai")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".codex-zai").join(CONFIG_TOML_FILE), "foo =").await?;
