...
def main() -> int:
    ...
    codex_home = Path(os.environ["USERPROFILE"]) / ".codex-zai"
    ...
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .codex-zai\\policy.json"], WS_ROOT)
    add("WS: .codex-zai cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .codex-zai policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
